  ...
  return (
    ...
    >
      <div className="max-w-5xl mx-auto flex items-center h-14 gap-4">
        ...
        {/* Liens visibles sur grand écran, cachés sur mobile (menu hamburger à la place) */}
        <ul className="hidden md:flex gap-5 font-body text-sm whitespace-nowrap overflow-x-auto flex-1" style={{ color: theme.textMuted }}>
          {NAV_IDS.map((id, i) => (
            <li key={id}>
              <a href={`#${id}`} onClick={(e) => scrollToId(e, id)} className="hover:underline underline-offset-4">
                {items[i]}
              </a>
            </li>
          ))}
      ...
      {mobileOpen && (
        <div id="mobile-nav-menu" className="md:hidden border-t -mx-4 sm:-mx-8" style={{ borderColor: theme.border, backgroundColor: theme.bg }}>
